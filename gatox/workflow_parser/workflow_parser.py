        ...
        for jobname, job_details in self.parsed_yml['jobs'].items():
            if 'runs-on' in job_details:
                ...
                if 'self-hosted' in runs_on:
                    ...
                    matrix_match = matrix_key_regex.search(runs_on)
                    if matrix_match:
                        ...
                        if any(key not in github_hosted_labels and not larger_runner_regex.match(key) for key in os_list):
                            sh_jobs.append((jobname, job_details))
                elif isinstance(runs_on, list):
                    if all(label in github_hosted_labels or larger_runner_regex.match(label) for label in runs_on):
                        continue
                    sh_jobs.append((jobname, job_details))
                elif isinstance(runs_on, str):
                    if runs_on in github_hosted_labels or larger_runner_regex.match(runs_on):
                        continue
                    sh_jobs.append((jobname, job_details))
