    ...
    def get_workflow_ymls(repos: list):
        ...
        for i in range(0, (len(repos) // 50) + 1):
            ...
            query = {
                ...
                "query": GqlQueries.GET_YMLS_ENV if any(repo.can_push() for repo in repos[i:top_len]) else GqlQueries.GET_YMLS,
                "variables": {
                    "node_ids": [
                        repo.repo_data['node_id'] for repo in repos[i:top_len]
                    ]
                }
            }
